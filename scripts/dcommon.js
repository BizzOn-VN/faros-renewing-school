...
App.Dev = function(){
    ...
    var register = function(){
        if (jQuery('form#ggcontact').valid() && flag) {
            ...
            jQuery.ajax({
                type : 'GET',
                url: 'https://script.google.com/macros/s/AKfycbwFhGZomW4w2DPFIBl1e4I21tp_QupVUcsgrvqSgpm19V9SOIhOIlGCBsWThO_Nrag/exec',
                dataType:'json',
                crossDomain : true,
                data : data,
                success : function(data)
                {
                    if(data == 'false')
                    {
                        ...
                        flag = 1;
                        jQuery('#gg-submit').val('Đăng ký tham gia');

                        if (data.result == "success") {
                            $('#reg-name').text($('#user-reg-name').val());
                            $('#reg-phone').text($('#user-reg-phone').val());
                            $('#phone-copy').text($('#user-reg-phone').val());
                            $('#reg-email').text($('#user-reg-email').val());

                            money = 300000;
                            money = money - (global_discount * money / 100);
                            $('#reg-money').html(vnd(money) + '<sup>vnđ</sup>');

                            qr_url = 'https://qr.sepay.vn/img?bank=ACB&acc=646664668&template=compact&des='+$('#user-reg-phone').val()+' RS&amount=';
                            $('#qr-code').attr('src', qr_url + money);
                            $('#download-qrcode').attr('href', qr_url + money);

                            jQuery('form#ggcontact')[0].reset();

                            gtag('event', 'dangKyThamDu');
    ...
    var getCurrentDate = function() {
        ...
        var maxDate = new Date('2025-11-20');
        if (today <= maxDate) {
            global_discount = 30;
        }
